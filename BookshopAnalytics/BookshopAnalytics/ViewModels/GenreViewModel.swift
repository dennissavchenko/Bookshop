...
/// A view model responsible for managing genres
///
...
@MainActor
@Observable
class GenreViewModel: @preconcurrency SearchViewModelProtocol {
    /// List of genres loaded from the API
    var briefEntities: [BriefEntity] = []
    
    /// Status code of the response
    var statusCode: Int?
    
    /// Loads the data for all genres from the API.
    func fetchBriefEntities(searchTerm: String) async {

        guard let url = URL(string: "http://localhost:5084/api/genres/search?searchTerm=\(searchTerm)") else {
