...
/// A view model responsible for managing authors
///
...
@MainActor
@Observable
class AuthorViewModel: @preconcurrency SearchViewModelProtocol {
    /// List of authors loaded from the API
    var briefEntities: [BriefEntity] = []
    
    /// Status code of the response
    var statusCode: Int?
    
    /// Loads the data for all authors from the API.
    func fetchBriefEntities(searchTerm: String) async {

        guard let url = URL(string: "http://localhost:5084/api/authors/search?searchTerm=\(searchTerm)") else {
